...
if __name__ == '__main__':
    ...
    model_path = "ppo_blood_vessel.zip"
    if os.path.exists(model_path):
        try:
            ...
            model = PPO(
                ...
                # Entropy coefficient (set to 0 for stability)
                ent_coef=0.0,
        ...
        model = PPO(
            ...
            gae_lambda=0.95,     # GAE parameter for advantage estimation
            clip_range=0.2,      # Clipping range for PPO
            ent_coef=0.0,        # Entropy coefficient (set to 0 for stability)
            tensorboard_log="./tensorboard_logs/"  # Enable TensorBoard logging
        )
    ...
    run_id = f"PPO_{int(time.time())}"  # Unique run ID based on timestamp
    ...
    try:
        ...
        model.save(f"ppo_blood_vessel_new_{run_id}")
        env.close()
        eval_env.close()
